...
export async function validateLocalesAreSupported(locales: string[], res: Response) {
    ...
    if (unsupportedLocales.length > 0) {
        let joiError = getJoiErr(`localizedVersions.${unsupportedLocales[0]}`, `Next locales are not supported ${unsupportedLocales.join(',')}. Either change request or change ${SettingKeys.I18nSupportedLocales} setting.`);
        res.status(422);
