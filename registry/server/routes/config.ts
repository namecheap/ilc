   ...
   try {
       ...
       routes.forEach(routeItem => {
           ...
           if (routeData === undefined) {
               ...
               const domainName = routeItem.domainId ? routerDomains.find(
                   ({ id }) => id === routeItem.domainId
               )?.domainName : null;

               routeItem.domainName = domainName || '*';
               delete routeItem.domainId;

               routeData = Object.assign({
                   slots: {},
                   meta: {},
               }, _.omitBy(_.pick(routeItem, ['routeId', 'route', 'next', 'template', 'specialRole', 'domainName']), _.isNull));
