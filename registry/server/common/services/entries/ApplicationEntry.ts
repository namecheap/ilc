...
export class ApplicationEntry implements Entry {
    ...
    public async patch(params: unknown, { user }: CommonOptions): Promise<App> {
        ...
        await db.versioning(user, { type: EntityTypes.apps, id: this.identifier }, async (trx) => {
            await db(Tables.Apps)
                .where({ name: this.identifier })
                .update(this.stringifyEntityValues(appEntity))
                .transacting(trx);
        });

        const [updatedApp] = await db.select().from(Tables.Apps).where('name', this.identifier);
    ...
    public async create(appDTO: App, { user }: CommonOptions) {
        ...
        await db.versioning(user, { type: EntityTypes.apps, id: appEntity.name }, async (trx) => {
            await db(Tables.Apps).insert(this.stringifyEntityValues(appEntity)).transacting(trx);
        });

        const [savedApp] = await db.select().from(Tables.Apps).where('name', appEntity.name);
    ...
    public async upsert(params: unknown, { user, trxProvider }: CommonOptions): Promise<void> {
        ...
        await db.versioning(user, { type: EntityTypes.apps, id: appEntity.name, trxProvider }, async (trx) => {
            await db(Tables.Apps)
    ...
    private async verifyExistence(identifier: string) {
        const countToUpdate = await db(Tables.Apps).where({
