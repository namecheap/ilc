...
export async function up(knex: Knex): Promise<void> {
    if (!isMySQL(knex)) {
        ...
        await knex.raw(
            'CREATE UNIQUE INDEX route_unique ON routes("orderPos", "domainIdIdxble", namespace) WHERE namespace IS NOT NULL',
        );
    }
}
...
export async function down(knex: Knex): Promise<void> {
    ...
    if (!isMySQL(knex)) {
        await knex.schema.alterTable('routes', (table) => {
            table.dropIndex(['orderPos', 'domainIdIdxble', 'namespace'], 'route_unique');
        });
        // TODO remove condition WHERE namespace IS NOT NULL when everything migrated
