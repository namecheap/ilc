...
export async function down(knex: Knex): Promise<any> {
    if (isMySQL(knex)) {
        return knex.schema.alterTable('routes', table => {
            table.integer('orderPos', 10).notNullable().unique('routes_orderpos_unique').alter();
            table.string('route', 255).notNullable().alter();
            table.boolean('next').notNullable().defaultTo(false).alter();
        });
    } else {
        return Promise.resolve()
